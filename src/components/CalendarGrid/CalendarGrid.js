...
const CalendarGrid = ({
  ...
  const { modalIsOpen, currentEvent, handleClose, handleOpen, totalDays } =
    useCalendarGridHook();
        ...
        return (
          ...
          >
            ...
            {events
              ...
              .filter(
                ev =>
                  moment(ev.date).format('X') >= day.format('X') &&
                  moment(ev.date).format('X') <=
                    day.clone().endOf('day').format('X')
              )
              .map(ev => (
                <DayTask key={ev.id} onClick={() => handleOpen(ev.id)}>
                  {ev?.title}
                </DayTask>
              ))}
          </GridCell>
